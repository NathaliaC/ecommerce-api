...
RSpec.describe 'Admin V1 Products as :admin', type: :request do
  ...
  context 'GET /products' do
    ...
    let!(:products) { create_list(:product, 10, categories:) }
    ...
    context 'with pagination params' do
      ...
      let(:pagination_params) { { page:, length: } }
  ...
  context 'DELETE /products/:id' do
    ...
    let!(:product) { create(:product, productable:) }
    let(:url) { "/admin/v1/products/#{product.id}" }
    ...
    it 'removes all associated product categories' do
      product_categories = create_list(:product_category, 3, product:)
      delete url, headers: auth_header(user)
